...
namespace MagicVilla_VillaAPI.Controllers.v2
{
    ...
    public class VillaAPIController : ControllerBase
    {
		...
		public VillaAPIController(IVillaRepository dbVilla, IMapper mapper, ImageService imageService)
        {
            _dbVilla = dbVilla;
            _mapper = mapper;
        ...
        public async Task<ActionResult<APIResponse>> CreateVilla([FromForm] VillaCreateDTO createDTO)
        {
            try
            {
				...
				if (createDTO.File != null)
				{
					...
					villa.ImageUrl = imageResult.SecureUrl.ToString();
					villa.PublicId = imageResult.PublicId;
				}
		...
		public async Task<ActionResult<APIResponse>> UpdateVilla(int id, [FromForm] VillaUpdateDTO updateDTO)
		{
			try
			{
				...
				{
					var imageUploadResult = await _imageService.AddImageAsync(updateDTO.Image);

					if (imageUploadResult.Error != null)
						return BadRequest(new ProblemDetails { Title = imageUploadResult.Error.Message });

					if (!string.IsNullOrEmpty(model.PublicId))
						await _imageService.DeleteImageAsync(model.PublicId);

					model.ImageUrl = imageUploadResult.SecureUrl.ToString();
